...
namespace LanguageServer.Services
{
    ...
    public class StepFactoryStoreFactory : IAsyncFactory<(StepFactoryStore stepFactoryStore, IExternalContext externalContext)>
    {
        ...
        async Task<(StepFactoryStore stepFactoryStore, IExternalContext externalContext)> Create(SCLLanguageServerConfiguration config)
            {
                var connectorRegistry = new ConnectorRegistry(LoggerFactory.CreateLogger<ConnectorRegistry>(), config.ConnectorRegistrySettings ?? ConnectorRegistrySettings.Reductech);
                ...
                Logger.LogInformation($"Connector Settings\r\nConfiguration Path: {settings.ConfigurationPath}\r\nConnector Path: {settings.ConnectorPath}");


                var connectorConfigurationDict = config.ConnectorSettingsDictionary?.Where(x => !string.IsNullOrWhiteSpace(x.Value.Id))
                    .ToDictionary(x => x.Key, x => x.Value);
            
                if (connectorConfigurationDict is null || connectorConfigurationDict.Count == 0)
                {
                    //load latest connectors from repository
                    var manager1 = new ConnectorManager(connectorManagerLogger, settings, connectorRegistry, new ConnectorConfiguration(), FileSystem);

                    var found = await manager1.Find(); //Find all connectors

                    connectorConfigurationDict = found.ToDictionary(x => x.Id, x => new ConnectorSettings() { Enable = true, Id = x.Id, Version = GetBestVersion(x.Version) });
                }

                var connectorManager = new ConnectorManager(connectorManagerLogger, settings, connectorRegistry, new ConnectorConfiguration(connectorConfigurationDict), FileSystem);

                var consoleAdapter = new LanguageServerConsoleAdapter(LanguageServerFacade);

                var defaultExternalContext = new ExternalContext(ExternalContext.Default.ExternalProcessRunner, ExternalContext.Default.RestClientFactory, consoleAdapter);

                var externalContextResult = await connectorManager.GetExternalContextAsync(defaultExternalContext.ExternalProcessRunner, defaultExternalContext.RestClientFactory, defaultExternalContext.Console, CancellationToken.None);
                ...
                var sfsResult = await connectorManager.GetStepFactoryStoreAsync(externalContextResult.Value, CancellationToken.None);
